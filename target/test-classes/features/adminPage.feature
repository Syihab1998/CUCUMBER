...
Feature: Search add and delete data on Admin Page
		...
		Scenario: Add new user data
			...
			And User select Admin as role & input "kim jong un" as employeeUserName & input "syihabcuy" as userName & input "s3Kol4HQA!*" as userPassword & input "s3Kol4HQA!*" as userRePass & click save button 
		...
		Scenario: User verify user data entered exist
			...
			And User input "syihab" as userName & click search button
		...
		Scenario: User looking for random data
			...
			And User input "cihui" as xUserNamee & click search button
		...
		Scenario: User register OAuth client
			...
			And User input "syihabbb" as oAuthId & input "syihab12" as oAuthSecret & input "[ https://orangehrm.github.io/orangehrm-api-doc ]." as oAuthUrl & click button save
